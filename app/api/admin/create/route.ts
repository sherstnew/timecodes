...
export async function POST(req: Request) {
    ...
    // If a file is provided, upload it to Yandex.Disk as before.
    if (file) {
        if (!apiKey)
            return NextResponse.json(
    ...
    // If no file but user provided metadata.videoPath, accept and store it directly.
    if (!file && metadata.videoPath) {
        const coll = await getCollection("markups");
        const doc = {
            title: metadata.title || metadata.videoPath,
            videoPath: metadata.videoPath,
            timecodes: metadata.timecodes || [],
            createdAt: new Date().toISOString(),
        };
        await coll.insertOne(doc as any);
        return NextResponse.json({ ok: true, path: metadata.videoPath });
    }

    return NextResponse.json({ error: "missing file or videoPath" }, { status: 400 });
}
