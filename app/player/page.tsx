...
function PlayerPageInner() {
  ...
  const currentTimecodes = useMemo(() => {
    ...
    return (data.timecodes as any[]).filter(
      (timecode) =>
        timecode.timeStart <= currentTime && timecode.timeEnd >= currentTime
    );
  }, [data, currentTime]);
              ...
              {currentTimecodes.map((timecode: any) => (
                ...
                >
                  ...
                  <AccordionContent className="pb-5">
                    <EditorRenderer serialized={timecode.text} />
                  </AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
                ...
                <AccordionTrigger>
                  <div className="flex items-center justify-between w-full">
                    <div className="text-sm text-muted-foreground mr-3">
                      {fmt(timecode.timeStart)} - {fmt(timecode.timeEnd)}
                    </div>
                    <div className="flex-1 text-left">{timecode.title}</div>
                  </div>
                </AccordionTrigger>
                  ...
                  <Button
                    onClick={() => {
                      if (playerRef.current) {
                        playerRef.current.setCurrentTime(timecode.timeStart);
...
export default function PlayerPage() {
  return (
    <Suspense fallback={<div className="p-6">Загрузка страницы...</div>}>
      <PlayerPageInner />
    </Suspense>
  );
}